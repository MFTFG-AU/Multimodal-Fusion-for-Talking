...
class LatentDiffusion(DDPM):
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox', 'audio']:
                    xc1 = batch['audio_smooth'][0]
                    xc2 = batch['reference_img'][0]
                    xc3 = batch['landmarks'][0]
                    xc4 = batch['image_mask'][0]
                    xc5 = batch['au'][0]
                    xc6 = batch['au1'][0]
                    xc7 = batch['img1'][0]
    ...
    def encode_first_stage(self, x):
        if hasattr(self, "split_input_params"):
            ...
            return self.first_stage_model.encode(x.cuda())

    def shared_step(self, batch, **kwargs):
        x, c_audio, c_lipID, c_ldm, c_mask,gt_aus1,img1 = self.get_input(batch, self.first_stage_key)
        #print(c_lipID.shape)
        loss = self(x, c_audio, c_lipID, c_ldm, c_mask,gt_aus1,img1)
        return loss

    def forward(self, x, c_audio, c_lipID, c_ldm, c_mask,gt_aus1,img1, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                ...
                #mask=[]
                #imag_real_mask1=[[] for i in range(len(img1))]
                #imag_real_mask=[[] for i in range(len(img1))]
                #print(gt_aus1.shape)[2, 55, 5]
                #for ttt in range(0,len(img1)):
                ...
                image_numpy_t_ = torch.tensor(image_numpy_t_).cuda()
                    #image_numpy_t_[0][56:112,0:112,0]=0
                mask=image_numpy_t_[0]
                mask=mask
                #print(input_images.shape,mask.shape)torch.Size([2, 55, 3, 112, 112]) torch.Size([2, 112, 112, 1])
                imag_real_mask=[]
                imag_real_mask1=[]
                c_lipID1=c_lipID.cuda()
                ...
                #print(c_audio.shape)  55 1 12 25 1
                rnn_feature10,AU10_feature=audio2AU10_model(c_audio.permute(0,4,1,2,3))  
                rnn_feature14,AU14_feature=audio2AU14_model(c_audio.permute(0,4,1,2,3))
                rnn_feature20,AU20_feature=audio2AU20_model(c_audio.permute(0,4,1,2,3))
                rnn_feature25,AU25_feature=audio2AU25_model(c_audio.permute(0,4,1,2,3))
                rnn_feature26,AU26_feature=audio2AU26_model(c_audio.permute(0,4,1,2,3))
                AU_feature=torch.cat([AU10_feature, AU14_feature, AU20_feature, AU25_feature, AU26_feature],dim=1)
                c_audio = self.get_learned_conditioning(c_audio.permute(4,1,0,2,3)[0][0], "audio")
                c_lipID = self.encode_first_stage(c_lipID.cuda())
                c_imag_real_mask = self.encode_first_stage(imag_real_mask.cpu())
                #c_imag_real_mask = self.get_learned_conditioning(imag_real_mask, "landmark")
                c_imag_real_mask1 = self.encode_first_stage(imag_real_mask1.cpu())
                c_ldm = self.get_learned_conditioning(c_ldm, "landmark")
                c_mask = self.encode_first_stage(c_mask)
            if self.shorten_cond_schedule:
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        return self.p_losses(x, c_audio, c_imag_real_mask,c_imag_real_mask1, c_ldm, c_mask, t,AU_feature, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond_audio, cond_lip,cond_lip1, cond_ldm, cond_mask, t,AU_feature, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output = self.apply_model(x_noisy, t, [cond_audio, cond_lip,cond_lip1, cond_ldm, cond_mask,AU_feature])#use the unet for denoising
        #model_output = self.apply_model(x_noisy, t, [cond_audio, cond_lip, cond_ldm, cond_mask])
        #model_output = self.apply_model(x_noisy, t, [cond_audio, cond_lip, cond_mask])#use the unet for denoising
    ...
    def p_mean_variance(self, x, c, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        AU_feature = c['AU_feature']
        model_out = self.apply_model(x, t_in, [cond_audio, cond_lip,cond_lip1, cond_ldm, cond_mask,AU_feature], return_ids=return_codebook_ids)#use the unet for denoising
    ...
    @torch.no_grad()
    def log_images(self, batch, N=55, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        z, c_audio, c_lip, c_ldm, c_mask, x, xrec, xc_audio, xc_lip,gt_aus1,img1 = self.get_input(batch, self.first_stage_key,
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                ...
                c = {'audio': c_audio, 'lip': c_imag_real_mask[:N],'lip1':c_imag_real_mask1[:N], 'ldm': c_ldm, 'mask_image': c_mask,'AU_feature':AU_feature[:N]}
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        self.tacn = TemporalConvNet(396, num_channels)
        self.fc1 = nn.Linear(3*28*28, 28*4)
        self.fc2 = nn.Linear(28*4, 4)
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            #print(c_crossattn[0].shape,c_crossattn[1].shape,c_crossattn[2].shape,c_crossattn[3].shape,c_crossattn[4].shape,c_crossattn[5].shape)

            f= c_crossattn[1].contiguous().view(c_crossattn[1].shape[0], -1)
            f1=self.fc1(f)
            f2=self.fc2(f1)
            cc = torch.cat([c_crossattn[0],f2,c_crossattn[3],c_crossattn[5]], 1)
            #cc = torch.cat([c_crossattn[0]], 1)
            #print("cc:",cc.shape)55 392

            self.tacn = self.tacn.cuda()
            cc=self.tacn(cc.unsqueeze(0).permute(0,2,1).cuda())
            
            cc = torch.unsqueeze(cc, 1)
            cc=cc[0].permute(2,0,1)
            #print(cc.shape)#55 1 392
            #print("xc:",x.shape,c_crossattn[3].shape, c_crossattn[1].shape)
            xc = torch.cat([x,  c_crossattn[4],  c_crossattn[2]], dim=1)
            #print("xc:",xc.shape)
            #cc = torch.cat([c_crossattn[0]], 1)
            #cc = torch.unsqueeze(cc, 1)
            #xc = torch.cat([x, c_crossattn[2], c_crossattn[1]], dim=1)
            ...
            cc = c_crossattn[0]
            out = self.diffusion_model.cuda()(x, t, y=cc)
        else:
            raise NotImplementedError()
